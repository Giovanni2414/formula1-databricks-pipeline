...
#        spark.sql(f"DELETE FROM f1_presentation.constructor_standings WHERE race_year = {race_id_list.race_year}")
        
#final_df.write.saveAsTable("f1_presentation.constructor_standings", format="delta", mode="append")
...
# MAGIC %sql
# MAGIC SELECT race_year, COUNT(1) FROM f1_presentation.constructor_standings GROUP BY race_year;
